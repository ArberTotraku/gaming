...
function getPlayerCredit() {
  if (firstBet < 10) {
    ...
    if (questionForMoney === "yes") {
      firstBet = Number(rs.question("How much then?"));
    } else {
      console.log("Sorry we don't accept small cash on our home");
      process.exit();
    }
    bettingScore = rs.question(` Do you really want to bet ${firstBet}€?`);
    if (bettingScore === "yes") {
      console.log(`Ok ${nameOfPlayer} your credit is ${firstBet}€`);
    }
  }
  // return playerCredit;
}
